(() => {
  ...
  function normalizeRecord(row, headers, rawHeaders, index) {
    ...
    const usedIndexes = new Set(Object.keys(FIELD_MAP).map(getIndex).filter((headerIndex) => headerIndex >= 0));
    const extraFields = headers.map((header, headerIndex) => {
      const value = clean(row[headerIndex]);
      if (!value || usedIndexes.has(headerIndex)) return null;
  ...
  function dashboardCardTemplate(record) { return `<button class="card" type="button" data-record-id="${escapeHtml(record.id)}"><h3>${escapeHtml(record.name)}</h3><p>${escapeHtml([record.company, record.productCode ? `Code ${record.productCode}` : "", record.use].filter(Boolean).join(" - "))}</p><p><strong>Storage:</strong> ${escapeHtml(record.storageLocation || "Not listed")}</p><p><strong>Review:</strong> ${escapeHtml(record.lastReviewDate || record.revisionDate || "Not listed")}</p><div class="badges"><span class="badge ${riskClass(record.risk)}">${escapeHtml(record.risk)}</span>${hasWebSds(record) ? `<span class="badge badge-linked">SDS linked</span>` : `<span class="badge badge-warning">SDS link missing</span>`}</div></button>`; }
  ...
  function showDetail(record) {
    ...
    panel.innerHTML = `<article class="detail" role="dialog" aria-modal="true"><header class="detail-header"><div><p class="eyebrow">SDS record</p><h2>${escapeHtml(record.name)}</h2><p>${escapeHtml([record.company, record.productCode ? `Code ${record.productCode}` : "", record.use].filter(Boolean).join(" - "))}</p></div><button class="close" type="button" aria-label="Close">Close</button></header><div class="detail-body"><section class="info-block"><h3>Product Information</h3>${definitionList([["Company", record.company], ["Product code", record.productCode], ["Use", record.use], ["Storage location", record.storageLocation], ["Location", record.location]])}</section><section class="info-block"><h3>SDS Details</h3>${definitionList([["SDS available", record.sdsAvailable], ["SDS #", record.sdsNumber], ["Version #", record.version], ["Issue date", record.issueDate], ["Revision / issue date", record.revisionDate], ["Supersedes date", record.supersedesDate], ["Approved by", record.approvedBy], ["Last review date", record.lastReviewDate]])}${record.sdsLink && /^https?:\/\//i.test(record.sdsLink) ? `<a class="link-button" href="${escapeHtml(record.sdsLink)}" target="_blank" rel="noreferrer">Open SDS</a>` : `<p>No web SDS link is available in the data feed.</p>`}</section><section class="info-block"><h3>Safety Reference</h3>${definitionList([["Composition", record.composition], ["HFRP / NFPA", record.hfrp], ["Risk level", record.risk], ...record.extraFields])}</section></div></article>`;
